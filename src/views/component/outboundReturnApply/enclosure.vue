            ...
            <template slot-scope="scope">
              <el-tooltip class="item" effect="dark" :content="`${scope.row.length || 0}`" placement="top">
                <div class="mask-td number-input">
                  ...
                  <el-input-number v-model="scope.row.length" controls-position="right" :min="0" :step="1" :precision="3" size="mini" @change="() => {scope.row.rules.length = false;calcToatlLength(scope.row);}" />
                </div>
              </el-tooltip>
            </template>
          </el-table-column>
          ...
          <template slot-scope="scope">
            <el-tooltip class="item" effect="dark" :content="`${scope.row.number || 0}`" placement="top">
              <div class="mask-td number-input">
                ...
                <el-input-number v-model="scope.row.number" controls-position="right" :min="1" :step="5" :precision="0" size="mini" @change="() => {scope.row.rules.number = false;calcToatlLength(scope.row);}" />
              </div>
            </el-tooltip>
          </template>
        </el-table-column>
          ...
          <template slot-scope="scope">
            <el-tooltip class="item" effect="dark" :content="`${scope.row.totalLength || '0'}`" placement="top">
              <div class="mask-td number-input">
                ...
                <el-input-number v-model="scope.row.totalLength" controls-position="right" :min="0" :step="0.5" :precision="3" size="mini" @change="scope.row.rules.totalLength = false" />
              </div>
            </el-tooltip>
          </template>
        </el-table-column>
...
import { MATERIAL_BASE_TYPE, MATERIAL_MEASURE, DAY_PICKER_OPTION, MATERIAL_INBOUND_TYPE, MATERIAL_INBOUND_NUM } from '@/utils/conventionalContent'
