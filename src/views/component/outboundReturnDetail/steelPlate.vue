    ...
    <div class="content-container">
      <el-table v-loading="tableLoading" :data="listDetail.detailList" max-height="600" style="width: 100%" border stripe>
        ...
        <el-table-column prop="taxIncludedAmount" :label="`总额 \n (元)`" align="center" min-width="90">
          <template slot-scope="scope">
            <el-tag v-if="scope.row.purchasePrice || scope.row.purchasePrice === 0" type="success" size="medium">{{ (scope.row.weight || 0 ) * (scope.row.purchasePrice || 0) | toFixed(2) }}</el-tag>
            <span v-else>/</span>
          </template>
        </el-table-column>
...
export default {
  name: 'OutboundReturnSteelPlateComponent',
...
<style scoped>
.outbound-return-record .el-table--medium td {
    padding: 5px 5px;
