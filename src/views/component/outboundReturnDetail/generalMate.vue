    ...
    <div class="content-container">
      <el-table v-loading="tableLoading" :data="listDetail.detailList" max-height="600" style="width: 100%" border stripe>
        ...
        <el-table-column prop="taxIncludedAmount" :label="`总额 \n (元)`" align="center" min-width="90">
          <template slot-scope="scope">
            <el-tag v-if="scope.row.purchasePrice || scope.row.purchasePrice === 0" type="success" size="medium">{{ (scope.row.number || 0 ) * (scope.row.purchasePrice || 0) | toFixed(2) }}</el-tag>
            <span v-else>/</span>
          </template>
        </el-table-column>
...
import { MATERIAL_BASE_TYPE, OUTBOUND_RETURN_VERIFY, OUTBOUND_RETURN_VERIFY_STATUS, MATERIAL_INBOUND_TYPE } from '@/utils/conventionalContent'
...
export default {
  ...
  data() {
    return {
      ...
      currentBaseType: MATERIAL_BASE_TYPE.material, // 钢板
