<template>
  ...
  <div class="page-container inbound-table">
    ...
    <div class="content-container" style="margin-bottom:80px">
      <el-table ref="poolTable" v-loading="tableLoading" :data="tableData" style="width: 100%" border stripe @selection-change="handleSelectionChange">
    ...
    >
      <el-form ref="handingOutForm" :label-position="'right'" label-width="80px" :model="handingOutForm" :rules="rules">
        ...
        <el-form-item label="物料尺寸">
          ...
          <el-tag v-if="currentMaterial.materialCode" size="small" style="margin-left:5px">{{ `长(m) * 宽(m) * 厚(mm)` }}</el-tag>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    submitMaterialMove: function(data) {
      ...
      materialMove({ type: this.moveType }, data).then(({ data, code, message }) => {
        if (code === 200) {
        ...
        if (code === 200) {
          if (data && data.data) {
            this.tableData = data.data.map(v => {
              v.isProvideMate = Number(v.storageListType) === 1 && !v.purchasePrice
              if (Number(v.storageListType) === 1 && !v.purchasePrice) {
