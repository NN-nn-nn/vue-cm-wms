<template>
  ...
  <div class="page-container">
      ...
      <el-tabs id="classTabs" v-model="currentTabId" :tab-position="'left'" style="min-height: 200px;" @tab-click="changeTab">
        <el-tab-pane v-for="(item,i) in classList" :key="i" :label="item.className + ' | ' + item.classCode" :name="`${item.id}`">
          <div class="filter-container">
            ...
            <!-- 右侧box -->
            <div class="filter-right-box">
              <!-- <el-button type="primary" size="medium" @click="openAddDlg">添加材质</el-button> -->
            </div>
          </div>
          <div class="content-container">
            <el-table
              ...
              <el-table-column prop="detailName" label="材质/型号/品种" align="center" width="220">
                <template slot-scope="scope">
                  <template v-if="scope.row.edit">
                    <div class="edit-item">
                      <el-input v-model="scope.row.detailName" class="edit-input" size="small" @input="validateName(scope.row.detailName, scope.row.id)" />
    ...
    <!-- 其他模块（例如弹窗等） -->
    <el-dialog title="添加物料种类" :visible.sync="classDlgVisible" width="500px" @closed="handleClose('classForm')">
...
export default {
  ...
  methods: {
    ...
    getMaterialList: function() {
      ...
      fetchMaterialList({ id: this.typeId }).then(res => {
    ...
    confirmEdit(row) {
      ...
      // updateDeviceLabel(row.id, row.name).then(
      //   response => {
      //     if (response.data.resultCode === 200) {
      //       row.edit = false
    ...
    deleteTip(status, item) {
      const tipName = status === 'class' ? item.className : item.detailName
